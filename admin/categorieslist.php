...
$id = $_GET["id"]; 
$sil = mysql_query("Delete From kategori where id='$id'");
...
?></div>
  ...
  $otels=mysql_query("select * from kategori WHERE cat='0'  order by baslik ASC   ");
  	...
  	while($otell=mysql_fetch_array($otelss)){

		echo '<p><font size="2px">------ '.$otell[baslik].' (<a href="categoryedit.php?id='.$otell[id].'">Düzenle</a> / <a href="?l=delete&id='.$otell[id].'">Sil</a>)</font></p>';
		
		
	  } ?></td>

                                         
                                            <td  width="200px"class="center"><a href="categoryedit.php?id=<?=$otel['id']?>"<button class="btn btn-primary"><i class="fa fa-edit "></i> Düzenle</button></a>
